...
action :create do
  ...
  if !@current_resource.exists #Only create if it does not exist.
    ...
    Chef::Log.error "DNS record already exists. Not Creating."
...
action :delete do
  ...
  if @current_resource.exists #Only delete if it exists.
    ...
    Chef::Log.error "DNS record does not exist. Not deleting."
...
def load_current_resource
  ...
  @current_resource = Chef::Resource::Dns.new(@new_resource.name)
